...
class StructuralPattern {
  ...
  static void dartComposite() {
    /**
     * We create a largeGiftBox and add three items to it. Next, we create smallGiftBox
     * with a single item inside. Then we add the smallGiftBox into the largeGiftBox,
     * so the largeGiftBox now contains three items and smallGiftBox. When we print out
     * the price and weight of the largeGiftBox, the result is an aggregate of all prices
     * and weights contained therein.
     */
  ...
  static void dartDecorator() {
    /**
     * First, we create a square and print the output from its draw() method, which
     * will be the shape's name alone. To create a green square, we construct an
     * instance of GreenShapeDecorator and pass it the square. When we draw the green
     * square, we see that the square has been decorated. One of the strengths
     * of the Decorator pattern is the ability to apply as many decorators as we wish
     * to any object, so we can add some blue to the green square, resulting in a square
     * with both color decorations.
     */
    final square = Square();
    print(square.draw());
